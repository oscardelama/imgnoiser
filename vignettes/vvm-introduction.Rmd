...
In the `imgnoiser` package most of those variables are read-only. However, technically its is possible to write or set values on them:
...
my.vvm$digest('crop_', 84, test.data.folder)
```
...
my.vvm$digest('crop_', samples.count, './samples/')
#> 84 image samples were successfully processed.
```
